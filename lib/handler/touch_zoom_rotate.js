...
function makeMovement(map, { rotationDisabled, aroundCenter }, e) {
  ...
  function onMove(e) {
    ...
    if (gestureIntent) {
      // zoom and rotate events are always fired by Map
      map.fire(`${gestureIntent}start`, { originalEvent: e });
      // move events are always fired by Map
  ...
  function onFrame(e, { vector, scale, center }) {
    ...
    // zoom and rotate events are always fired by Map
    map.fire(gestureIntent, { originalEvent: e });
    // move events are always fired by Map
  ...
  function onEnd(e) {
    ...
    // zoom and rotate events are always fired by Map
    map.fire(`${gestureIntent}end`, { originalEvent: e });
 ...
 */
export default function touchZoomRotateHandler(map) {
  let enabled = false;
