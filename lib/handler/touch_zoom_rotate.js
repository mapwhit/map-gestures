...
import { initFireEvent } from '../util/events.js';
...
function makeMovement(map, { rotationDisabled, aroundCenter }, e, fireEvent) {
  ...
  const inertia = makeInertia(calculateInertia);
  ...
  function onMove(e) {
    ...
    if (gestureIntent) {
      fireEvent(`${gestureIntent}start`, e);
 ...
 */
export default function touchZoomRotateHandler(map, options) {
  options = { ...options };
  const fireEvent = (options._fireEvent ??= initFireEvent(map, options));
  ...
  function onStart(e) {
    ...
    movement = makeMovement(map, { rotationDisabled, aroundCenter }, e, fireEvent);
