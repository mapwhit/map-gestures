...
import { initFire, initFireEvent, MapMouseEvent, MapTouchEvent, MapWheelEvent } from './util/events.js';
 ...
 * @param {boolean} [options.dispatchOwnEvents=true] If `false`, the map will be used to dispatch events, if `true`, the map gestures module will dispatch events by itself.
 */
export default function bindMapGestures(map, options) {
  ...
  let mouseDown = false;
  let startPos = null;
  const fireEvent = initFire(map, options);
  options._fireEvent = initFireEvent(map, options, fireEvent);
