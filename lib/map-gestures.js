...
export default function bindMapGestures(map, options) {
  ...
  function isMoving() {
    return (
      map.dragPan.isActive() || map.dragRotate.isActive() || map.touchZoomRotate.isActive() || map.scrollZoom.isActive()
    );
  }

  function isZooming() {
    return map.touchZoomRotate.isActive() || map.scrollZoom.isZooming();
  }

  function isRotating() {
    return map.touchZoomRotate.isActive() || map.dragRotate.isActive();
  }
}
