...
export function initFire(map, options) {
  return (dispatcher => dispatcher.fire.bind(dispatcher))(options.dispatchOwnEvents ? new Evented() : map);
}

export function initFireEvent(map, options, fireEvent = initFire(map, options)) {
  return (type, e, data = {}) => fireEvent(new Event(type, e ? { originalEvent: e, ...data } : data));
}
